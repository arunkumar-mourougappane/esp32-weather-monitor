...
### Stale Cache Badge (fetch failed, cached data displayed)

`showStaleBadge(fetchTime)` partial-refreshes Y 910–960 at `epd_fastest`. Called after
`renderActivePage()` when `rtcLastError == kErrWeatherFail || kErrWiFiFail` and
`rtcCachedWeather.valid` is true. Inverted background (black strip) distinguishes it from
the normal refreshing badge.

Age is computed as `(time(nullptr) - fetchTime) / 60` minutes:
- `< 60 min` → `"Xm ago"`, e.g. `"45m ago"`
- `≥ 60 min` → `"Xh ago"`, e.g. `"2h ago"`
- timestamp unavailable → `"unknown age"`

```
 910 ████████████! Cached  |  Last sync 2h ago█████████████ ← inverted strip (black bg, white text)
 960 └──────────────────────────────────────────────────────┘
```
...
| Y | X | Content | Notes |
|---|---|---------|-------|
| 910 | 0 | `fillRect(0, 910, 540, 50, TFT_BLACK)` | Inverted strip background |
| 922 | 270 (centre) | `"! Cached  |  Last sync Xh ago"` | FreeSans9pt7b ×1, white text, TC_DATUM |
